...
Existing uses of GitHub Classroom in CDIO and computing-education contexts focus on automated grading and assignment distribution (\cite{githubsurvey}), leveraging test frameworks and repository templates to check correctness rather than to support collaborative review. 
...
While the broader use of GitHub in education is well documented, fewer studies examine PR-based workflows as primary assessment artifacts in project-based courses.
...
Over successive offerings, PRs became smaller and better scoped, documentation improved, and review discussions became more substantive. This progression is also reflected in rubric-based assessment data from the second-year IE course (cf.~Table~\ref{tbl:IE-rubric}), where GitHub practices are introduced. Normalized scores across eight team assignments (23 student observations) show a positive trend in both GitHub usage (slope = 0.019, $p = 0.023$) and teamwork and participation (slope = 0.047, $p < 0.001$), indicating increasing adoption of collaborative workflows and more active engagement in shared development practices.
These trends are consistent with observed improvements relative to earlier course iterations without structured PR workflows in which collaboration workflows were not explicitly structured.

\input{include/rubric}

\section{Alignment with CDIO Standards}
This implementation aligns primarily with CDIO Standards 7, 8, and 11, with partial alignment to Standard 5 (\cite{malmqvist2020standards3}). 
